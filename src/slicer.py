"""Slice YOLO datasets into overlapping patches for high-accuracy training.

Slices large microscopy images into smaller overlapping patches and
remaps bounding-box labels into the new coordinate system.

Usage (as module):
    from src.slicer import slice_dataset

Usage (CLI):
    python -m src.slicer
    python -m src.slicer --input datasets --output datasets_sliced --slice-size 512 --overlap 0.2
"""
...
def get_yolo_boxes(label_path: str | Path, img_w: int, img_h: int) -> list:
    """Parse YOLO label file and return boxes in absolute pixel coords.

    Returns:
        List of [cls_id, x1, y1, x2, y2] in absolute pixels.
    """
    boxes = []
...
def calculate_intersection_and_format(
    box: list,
    win_x: int,
    win_y: int,
    win_size: int,
    ...
    """Compute intersection of a box with a window and format as YOLO line.

    Returns:
        YOLO-format string or None if the box is too truncated.
    """
...
def process_image(
    ...
    """Slice a single image into patches and save those with annotations.

    Returns:
        (saved_count, skipped_count) tuple.
    """
...
def slice_dataset(
    input_dir: str | Path = "datasets",
    output_dir: str | Path = "datasets_sliced",
    slice_size: int = 512,
    overlap: float = 0.2,
    min_area: float = 0.5,
) -> tuple[int, int]:
    """Slice an entire YOLO dataset into overlapping patches.

    Args:
        input_dir: Root dataset directory with images/{train,val} and labels/{train,val}.
        output_dir: Output directory for sliced patches.
        slice_size: Patch size in pixels.
        overlap: Overlap ratio between patches (0 to 1).
        min_area: Minimum remaining area ratio to keep a bounding box.

    Returns:
        (total_new, total_skipped) tuple.
    """
    ...
    for split in ["train", "val"]:
        ...
        for img_path in img_paths:
            ...
            slices, skipped = process_image(img_path, lbl_path, out_img_dir, out_lbl_dir, slice_size, overlap, min_area)
            total_slices += slices
            total_skipped += skipped

    print(f"\n[SUCCESS] Generated {total_slices} new patched images (Skipped {total_skipped} existing) in {out_dir}")
    return total_slices, total_skipped
...
def main():
    ...
    slice_dataset(args.input, args.output, args.slice_size, args.overlap, args.min_area)
